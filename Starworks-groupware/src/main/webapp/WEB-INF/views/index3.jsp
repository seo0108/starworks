	...
	<div class="main-container">
		...
		<div class="main-content">
			...
			<div class="content">
				...
				<div class="row animate-on-load" style="animation-delay: 0.2s;">
					<!-- Card 1: 날짜/시간 -->
					<div class="col-md-4">
						<div class="card mb-4">
							<div class="card-header">
								<i class="bi bi-calendar-event me-2"></i>오늘의 날짜
							</div>
							<div class="card-body">
								<div class="welcome-title">좋은 하루 시작하세요!</div>
								<div class="welcome-subtitle">
									오늘도 화이팅입니다 <i class="fa-solid fa-hand-fist text-warning"></i>
								</div>
								<div id="datetime-container" class="mt-3">
									<!-- 날짜/시간 -->
									<div class="welcome-title" id="time-display">
										<span class="welcome-title" id="time-icon"></span> <span
											class="welcome-title" id="time-text">--:--:--</span>
									</div>
									<div class="welcome-subtitle" id="date-display">----년 --월
										--일 --요일</div>
								</div>
							</div>
						</div>
					...
					<div class="col-md-4">
                        ...
                        --:--:--
								...
								<div class="work-duration">
									오늘 근무시간: <span id="work-timer">00:00:00</span>
								</div>
								<div class="attendance-buttons mt-3">
									<button class="attendance-btn" id="clock-in-btn"
										${not empty attendanceRecord.workStartTime ? 'disabled' : ''}>
										<i class="bi bi-box-arrow-in-right me-2"></i>출근
									</button>
									<button class="attendance-btn" id="clock-out-btn"
										${(not empty attendanceRecord.workStartTime and empty attendanceRecord.workEndTime) ? '' : 'disabled'}>
					...
					<div class="col-md-4">
						<div class="card mb-4">
							<div class="card-header">
								<i class="bi bi-cloud-sun me-2"></i>오늘의 날씨
							</div>
							<div class="card-body">
								<div id="weather-box" style="color: #333; text-align: left;">
									<!-- 아이콘 -->
									...
									<div id="weather-text" style="line-height: 1.6;">
										<div id="weather-temp"
											style="font-size: 1.4rem; font-weight: 700;">--°C</div>
										<div id="weather-desc" style="font-size: 1rem;">날씨 정보를
											불러오는 중...</div>
										<div id="weather-detail"
											style="font-size: 0.9rem; opacity: 0.9;"></div>
										<div id="weather-sun"
											style="font-size: 0.85rem; margin-top: 3px;"></div>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="col-lg-8">
						...
						<div class="stats-grid mb-0">
							<div class="card stat-card animate-on-load"
								...
								<i class="bi bi-check-circle-fill stat-icon"></i> <a
									href="/projects/main">
							...
							<div class="card stat-card animate-on-load"
								...
								<i class="bi bi-pen-fill stat-icon"></i> <a
									href="/approval/main?filter=inbox">
							...
							<div class="card stat-card animate-on-load"
								...
								<i class="bi bi-calendar stat-icon"></i> <a
									href="/calendar/depart">
							...
							<div class="card stat-card animate-on-load"
								...
								<i class="bi bi-envelope-fill stat-icon"></i> <a
									href="/mail/list">
								...
								<c:if test="${not empty todayScheduleList }">
										...
										<c:if test="${not empty todaySchedule.startTime }">
											<div class="schedule-item">
												<div class="schedule-time me-4">
													${fn:substringAfter((todaySchedule.startTime), 'T')} -
													${fn:substringAfter((todaySchedule.endTime), 'T')}</div>
									...
									<c:forEach var="todaySchedule" items="${todayScheduleList }">
										<c:if test="${empty todaySchedule.startTime }">
											<div class="schedule-item">
												<div class="schedule-time ms-1 me-4">하루종일</div>
												<div class="schedule-content">
													<div class="schedule-title">${todaySchedule.scheduleNm }</div>
													<c:if test="${todaySchedule.scheduleType eq 'department' }">
														<div class="schedule-location">등록자 :
															${todaySchedule.userNm } ${todaySchedule.jbgdNm}
															(${todaySchedule.deptNm })</div>
													</c:if>
													<c:if test="${todaySchedule.scheduleType eq 'personal' }">
														<div class="schedule-location">${todaySchedule.description }</div>
													</c:if>
								...
								<c:choose>
									<c:when test="${not empty currentProjectList }">
										<c:forEach var="currentProject" items="${currentProjectList }">
											<a style="color: #607080"
												href="/projects/${currentProject.pk }">
												<div class="progress-item">
													...
													<div>종료일
														${fn:substringBefore((currentProject.endDate), 'T')}</div>
												</div>
											</a>
										</c:forEach>
									</c:when>
									<c:otherwise>
										<div class="my-5" style="text-align: center">참여중인 프로젝트가
											없습니다</div>
									</c:otherwise>
								</c:choose>
							</div>
					...
					<div class="col-lg-4">
						...
						<div class="card mb-4 animate-on-load"
							...
							<div class="card-body">
								<div class="quick-actions">
									<a href="/approval/main" class="quick-action"> <i
										class="bi bi-pencil-square"></i> <span>결재 홈</span>
									</a> <a href="/calendar/depart" class="quick-action"> <i
										class="bi bi-calendar-plus"></i> <span>일정 등록</span>
									</a> <a href="/projects/main" class="quick-action"> <i
										class="bi bi-easel2"></i> <span>나의 프로젝트</span>
									</a> <a href="/mail/send" class="quick-action"> <i
										class="bi bi-envelope-plus"></i> <span>메일 작성</span>
									</a>
								</div>
							</div>
						</div>
						...
						<div class="card mb-4 animate-on-load"
										...
										<a href="board/notice/${recentBoard.pk }">
											<div class="notice-item">
												...
												<div class="notice-meta">
													<span> ${recentBoard.crtUserVO.userNm } <%-- 			                                        	${recentBoard.crtUserVO.jbgdNm } --%>
							...
							<c:if test="${empty recentBoardList }">
								<div class="card-body">등록된 공지사항이 없습니다.</div>
							</c:if>
						</div>
						...
						<div class="card animate-on-load" style="animation-delay: 0.9s;">
							...
							<div class="card-body animate-on-load"
								style="animation-delay: 0.9s;">
								<div
									class="d-flex justify-content-between align-items-center mb-3 animate-on-load"
									style="animation-delay: 0.9s;">
									<span>전체 팀원</span> <strong>${departStatus.departCnt }명</strong>
								</div>
								<div
									class="d-flex justify-content-between align-items-center mb-3 animate-on-load"
									style="animation-delay: 0.9s;">
									<span>현재 근무 중</span> <strong>${departStatus.workingUserCnt }명</strong>
								</div>
								<div
									class="d-flex justify-content-between align-items-center mb-3 animate-on-load"
									style="animation-delay: 0.9s;">
									<span>휴가</span> <strong>${departStatus.vacationUserCnt }명</strong>
								</div>
								<div
									class="d-flex justify-content-between align-items-center animate-on-load"
									style="animation-delay: 0.9s;">
									<span>외근</span> <strong>${departStatus.businessTripCnt }명</strong>
								</div>
							</div>
						</div>
					</div>

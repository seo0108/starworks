 ...
 *  2025. 10. 20.	  홍현택				left-menu 상호작용 스크립트 추가
 *
 * </pre>
 */
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 폼 및 주요 요소 참조
    const form = document.getElementById('replyForm');                    // 메일 전송/임시저장 폼
    ...
    const editorTextarea = document.querySelector('#replyEditor');        // 본문 textarea (CKEditor 동기화 대상)
    ...
    const replyEditorError = document.getElementById('replyEditorError');

    // 에디터 및 자동완성 상태
    let editor5 = window.replyEditor;      // CKEditor5 인스턴스 (외부에서 주입)
    let selectedRecipients = [];           // 선택된 수신자 목록
    let acTimeout = null;                  // 자동완성 디바운싱 타이머 id

    // 원문 연계 정보 (답장 대상 등)
    const originalEmailContIdInput = document.querySelector('input[name="originalEmailContId"]');
    const originalEmailContId = originalEmailContIdInput ? originalEmailContIdInput.value : null;

    // 원발신자 정보를 초기 수신자로 세팅 (JSP에서 data-sender-id 제공)
    const originalSenderId = emailToInput?.dataset?.senderId;
    const originalSenderDisplayText = emailToInput?.value || '';
    if (originalSenderId) {
        selectedRecipients.push({
            userId: originalSenderId,
            userNm: originalSenderDisplayText.substring(0, originalSenderDisplayText.indexOf('(')),
            displayText: originalSenderDisplayText
        });
        if (emailToInput) {
    ...
    function validateForm() {
        ...
        if (editorContent === '' || editorContent === '<p></p>') {
            if (replyEditorError) {
                replyEditorError.textContent = '내용을 입력해주세요.';
                replyEditorError.style.display = 'block';
            }
            isValid = false;
        } else if (replyEditorError) {
            replyEditorError.style.display = 'none';
        }
    ...
    if (sendBtn) {
        sendBtn.addEventListener('click', async function (e) {
            ...
            submitWithAction((window.contextPath || '') + '/mail/reply', false);
        });
    }

    // 임시저장: 확인 모달 → 제출 (유효성 검사 강제하지 않음)
    if (saveDraftBtn) {
    ...
    if (editor5) {
        editor5.model.document.on('change:data', () => {
            if (replyEditorError) replyEditorError.style.display = 'none';
        });
    }
        ...
        acTimeout = setTimeout(() => {
            fetch((window.contextPath || '') + '/rest/comm-user/autocomplete?term=' + encodeURIComponent(term))
                ...
                .then(users => {
                    ...
                    users.forEach(user => {
                        // 이미 선택된 userId 필터링을 원하면 아래 주석 해제

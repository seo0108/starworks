...
<body>
		...
		<div class="page-content d-flex flex-wrap gap-3">
			...
			<div style="flex: 1 1 78%;" class="right-content">
				...
				<div class="outline-section">
					<div class="outline-title">${deptNm } 부서자료실</div>
					<div class="outline-subtitle">부서 문서를 관리합니다.</div>
				</div>
				...
				<section class="section">
					<div class="card" style="height: 680px;">
						<div class="card-body">
							<div
								class="row mb-3 justify-content-between align-items-center">
								<div class="col-md-6">
								...
								<div class="col-md-6 text-end">
									<button class="btn btn-info" id="batch-download">
										<svg xmlns="http://www.w3.org/2000/svg" width="16"
											height="16" fill="currentColor" class="bi-download"
											viewBox="0 0 16 16">
								  <path
								  ...
								  <path
									...
									<button class="btn btn-danger" id="batch-delete">
										<svg xmlns="http://www.w3.org/2000/svg" width="16"
											height="16" fill="currentColor" class="bi-trash"
											viewBox="0 0 16 16">
								  <path
										...
										<c:if test="${not empty departmentFileList }">
											<c:forEach items="${departmentFileList }"
												...
												<tr>
													...
													<td class="text-center">${departmentFile['userNm'] }</td>
													<td class="text-center">${fn:substringBefore(departmentFile['crtDt'], 'T')}</td>
													<td class="text-center">
														<c:set value="${departmentFile['fileSize'] }" var="fileSize" />
														<c:choose>
	...
	<!-- 파일 추가 모달 -->
	<div id="departFileModal" class="modal fade text-left" tabindex="-1"
			...
			<div class="modal-content">
				<!-- 파일 생성 폼 -->
					...
					<div class="modal-body">
						자료실 위치 <select id="folderSelect" name="folderSqn" class="form-select mb-3">
							<option value="">부서자료실</option>
							<!-- 문서등록 모달 - 폴더 select -->
						</select> 파일 첨부
						<form id="fileCreateForm" action="/rest/document-depart/department" method="post" enctype="multipart/form-data" class="dropzone" >
						</form>
					</div>
...
<%-- 				</form> --%>
	...
	<%@ include file="/WEB-INF/views/document/remove-confirm-modal.jsp" %>
		...
		<script type="text/javascript">
			...
			const dropzoneInit = () => {
			    const myDropzone = new Dropzone("#fileCreateForm", {
			    	url : "/rest/document-depart/department",
			        paramName: "fileList",
			        maxFilesize: 1000,
			        addRemoveLinks: true,
			        ...
			        init: function() {
			            ...
			            dz.on("queuecomplete", function(file) {
			                ...
			                if (dz.getQueuedFiles().length === 0 && dz.getUploadingFiles().length === 0) {
			                    const modalEl = document.getElementById('departFileModal');
		...
		<form method="post" id="deleteForm" action="/document/user/remove?type=depart"></form>
